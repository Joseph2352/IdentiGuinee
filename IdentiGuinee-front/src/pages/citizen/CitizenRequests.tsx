         ...
         {loading ? (
             <>
                {Array.from({ length: 3 }).map((_, i) => (
                  <div key={i} className="bg-surface-container-lowest border border-outline-variant/10 rounded-2xl shadow-sm p-6 relative overflow-hidden h-40 flex flex-col justify-between">
                     <div className="flex justify-between items-start">
                       <div><Skeleton className="h-3 w-16 mb-3" /><Skeleton className="h-6 w-48" /></div>
                       <div className="text-right flex flex-col items-end"><Skeleton className="h-4 w-12 mb-3" /><Skeleton className="h-3 w-20" /></div>
                     </div>
                     <div>
                       <div className="flex justify-between mb-2"><Skeleton className="h-3 w-20" /><Skeleton className="h-3 w-10" /></div>
